...
describe("#execute", () => {
  beforeEach(() => {
    ...
    createCoordinateService = new CreateCoordinateService(coordinatesRepository);
  });

  it("creates a coordinate when user exists", async () => {
    const xValue = "12.3213"
    const yValue = "32.1234"
    const coordinate = await createCoordinateService.perform({ collaboratorId: collaboratorModel.id, x: xValue, y: yValue });
